...
class TestLLMParserBlockSingleResponse:
    ...
    def test_missing_fields_partial_extraction(self, caplog):
        ...
        dataset = Dataset.from_dict({
            "llm_response": [{"content": "Hello world"}]  # Missing reasoning_content
        })
        ...
        assert "Requested fields ['reasoning_content'] not found in response" in caplog.text
    ...
    def test_multiple_missing_fields_warnings(self, caplog):
        ...
        dataset = Dataset.from_dict({
            "llm_response": [{"content": "Hello world"}]  # Missing reasoning_content and tool_calls
        })
        ...
        assert "Requested fields ['reasoning_content', 'tool_calls'] not found in response" in caplog.text
...
class TestLLMParserBlockListResponsesExpandTrue:
    ...
    def test_expand_list_responses(self):
        ...
        assert len(result) == 3
        assert result["test_parser_content"] == ["Response 1", "Response 2", "Response 3"]
        assert result["other_col"] == ["original_value", "original_value", "original_value"]
    ...
    def test_expand_multiple_samples(self):
        ...
        dataset = Dataset.from_dict({
            "llm_response": [
                [{"content": "Sample 1 Response 1"}, {"content": "Sample 1 Response 2"}],
        ...
        assert result["test_parser_content"] == [
            "Sample 1 Response 1", "Sample 1 Response 2", "Sample 2 Response 1"
        ]
    ...
    def test_expand_invalid_list_items(self, caplog):
        ...
        dataset = Dataset.from_dict({
            "llm_response": [[
                ...
                {"other_field": "value"},  # Dict without content field - will be skipped
        ...
        assert len(result) == 3
        assert result["test_parser_content"] == ["Valid response", "", "Another valid response"]
...
class TestLLMParserBlockListResponsesExpandFalse:
    ...
    def test_preserve_list_structure(self):
        ...
        assert len(result) == 1
        assert result["test_parser_content"][0] == ["Response 1", "Response 2", "Response 3"]
    ...
    def test_preserve_multiple_fields(self):
        ...
        assert result["test_parser_reasoning_content"][0] == ["Reasoning 1", "Reasoning 2"]
...
class TestLLMParserBlockErrorHandling:
    ...
    def test_invalid_input_type(self, caplog):
        ...
        dataset = Dataset.from_dict({
            "llm_response": ["not_a_dict_or_list"]
        })
    ...
    def test_none_content_handled_gracefully(self, caplog):
        ...
        dataset = Dataset.from_dict({
            "llm_response": [{"content": None, "role": "assistant"}]  # None content field
        })
    ...
    def test_none_reasoning_content_handled_gracefully(self, caplog):
        ...
        dataset = Dataset.from_dict({
            "llm_response": [{"reasoning_content": None, "role": "assistant"}]  # None reasoning_content field
        })
        ...
        assert "Reasoning content field is None, using empty string instead" in caplog.text
